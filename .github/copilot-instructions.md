...
NVIDIA NemoClaw is an open-source reference stack for running [OpenClaw](https://openclaw.ai) always-on assistants inside NVIDIA OpenShell sandboxes more safely. It provides CLI tooling, a blueprint for sandbox orchestration, and security hardening.
...
The project maintains a persistent, compounding knowledge base (the "LLM Wiki"). It lives alongside the codebase and should be kept up to date.

### Layout

| Path | Purpose | Ownership |
|------|---------|-----------|
| `wiki/WIKI.md` | Schema and conventions (loaded into agent system prompt) | Co-evolved by human + LLM |
| `wiki/index.md` | Content catalog — every page with a link and one-line summary | LLM maintains |
| `wiki/log.md` | Append-only chronological activity record | LLM maintains |
| `wiki/overview.md` | Living executive summary (max 3K words) | LLM maintains |
| `wiki/entities/`, `concepts/`, `sources/`, `analyses/` | Page categories | LLM writes, humans read |
| `wiki-raw/` | Immutable source documents — never modify after deposit | Humans deposit |

### Rules

- Raw sources in `wiki-raw/` are **immutable** — never edit after deposit
- Page slugs are lowercase, hyphenated: `my-topic-name.md`
- Use relative links between wiki pages
- Always add new pages to `wiki/index.md` in the same pass
- Bump the `updated` frontmatter field when editing a page

### Three Core Operations

1. **Ingest** — Drop a source into `wiki-raw/`, then ask the LLM to ingest it. It writes a source summary, creates/updates entity and concept pages, updates cross-references, `index.md`, and `log.md`.
2. **Query** — The agent reads `index.md` to locate pages, synthesizes an answer with citations, and optionally files the result as an analysis.
3. **Lint** — Periodic health check: finds contradictions, orphan pages, missing concepts, stale claims, and missing cross-references.
