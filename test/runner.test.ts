...
describe("regression guards", () => {
  ...
  it("keeps a single shellQuote definition in the root CLI codebase", () => {
    ...
    // Fork keeps bin/lib/runner.js alongside the canonical src/lib/runner.ts
    expect(defs).toHaveLength(2);
    expect(defs).toContain(path.join("bin", "lib", "runner.js"));
    expect(defs).toContain(path.join("src", "lib", "runner.ts"));
  });
